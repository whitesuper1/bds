    ...
    function wpestate_check_free_listing_expiration(){
        ...
        if($free_feat_list_expiration!=0 && $free_feat_list_expiration!=''){
            ...
            while ($prop_selection->have_posts()): $prop_selection->the_post();          
                ...
                //print get_the_title(); print' - ';
                $the_id=get_the_ID();
                //print '-'. get_the_date("Y-m-d",  $the_id ) ;
                //print '</br>';
...
if( !function_exists('wpestate_listing_set_to_expire') ): 
    function wpestate_listing_set_to_expire($post_id){
        ...
        $headers = 'From: No Reply <noreply@'.$_SERVER['HTTP_HOST'].'>' . "\r\n";
        $message  = __('A free listing has expired,','wpestate') . "\r\n\r\n";
        $message .= sprintf( __("Hello, One of your free listings on  %s has \"expired\". The listing is %s. Thank you!",'wpestate'), get_option('blogname'), get_permalink($post_id)) . "\r\n\r\n";

    wp_mail($user_email,
            sprintf(__('[%s] Free Listing expired','wpestate'), get_option('blogname')),
            $message,
            $headers);
    }
endif;
