...
class MY_PostWidgetCustomTTTech extends WP_Widget {
	/** constructor */
	function MY_PostWidgetCustomTTTech() {
		parent::WP_Widget(false, $name = __('TT-Tech - Recent Posts', CHERRY_PLUGIN_DOMAIN));
	}
	...
	function widget($args, $instance) {
		...
		echo $before_widget;
		if ( $title )
			echo $before_title . $title . ' '. $category. $after_title;
		...
		if ($tags) {
			...
			<li class="post-list_li clearfix">
				...
				<?php if(has_post_thumbnail()) {
					...
					$image   = aq_resize( $img_url, 132, 202, true ); //resize & crop img
				?>
				...
				<time datetime="<?php the_time('Y-m-d\H:i'); ?>"><?php echo get_the_date('d/m/Y'); ?></time>
				...
				<?php } ?>
				<a href="<?php the_permalink() ?>" class="btn btn-primary"><?php _e('more', CHERRY_PLUGIN_DOMAIN); ?></a>
			</li>
